...
def plotEigVec(fixed_list, x_ax, y_ax):
    ...
    plt.title('phase plane for eqation(3)')
...
def plotStateXY(x, y):
    ...
    plt.title('X state for eqation(3)')
    ...
    plt.title('Y state for eqation(3)')
    plt.xlabel('Time-step')
    plt.ylabel('Number of Y')
    plt.grid(True)
    plt.tight_layout()
...
if __name__ == '__main__':
    #fixed = ([0,0], [0, 2], [3, 0])
    fixed = [(0.0, 0.0), (0.0, 2.0), (1.5, 0,0)]
    initialPoints = ([0.1, 0.1], [1.1, 0.2], [3.0, 0.2], [4.5, 2.1])
